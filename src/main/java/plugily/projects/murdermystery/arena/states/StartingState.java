...
public class StartingState extends PluginStartingState {
  ...
  public void handleCall(PluginArena arena) {
    ...
    if (!pluginArena.isHideChances()) {
      for (Player player : arena.getPlayersLeft()) {
        pluginArena.getPlugin().getActionBarManager().addActionBar(player,
            new ActionBar((new MessageBuilder("IN_GAME_MESSAGES_ARENA_ROLE_CHANCES_ACTION_BAR")).asKey().player(player)
                .arena(pluginArena), ActionBar.ActionBarType.DISPLAY));
      }
    }
    ...
    if (arena.getTimer() == 0 || arena.isForceStart()) {
      ...
      // Clear cached contribution values before role assignment to ensure rolepass
      // takes effect
      pluginArena.getMurdererContributions().clear();
      pluginArena.getDetectiveContributions().clear();
      ...
      for (Player player : playersToSet) {
        new TitleBuilder("IN_GAME_MESSAGES_ARENA_PLAYING_ROLE_INNOCENT").asKey().player(player).arena(pluginArena)
            .sendPlayer();
      }

      arena.getPlugin().getDebugger().debug(
          "After: Arena: {0} | Detectives = {1}, Murders = {2}, Players = {3} | Players: Detectives = {4}, Murders = {5}",
          arena.getId(), maxdetectives, maxmurderer, arena.getPlayersLeft().size(), pluginArena.getDetectiveList(),
  ...
  private void addRole(Arena arena, Role role, Set<Player> playersToSet) {
    ...
    List<IUser> chancesRanking = getPlugin().getUserManager().getUsers(arena).stream()
        ...
        .sorted(Comparator.comparingInt(user -> arena.getContributorValue(role, user))).collect(Collectors.toList());
    ...
    for (int i = 0; i < amount; i++) {
      ...
      new TitleBuilder("IN_GAME_MESSAGES_ARENA_PLAYING_ROLE_" + roleName).asKey().arena(arena).player(userPlayer)
          .sendPlayer();
  ...
  private void getMaxRolesToSet(Arena arena) {
    ...
    arena.getPlugin().getDebugger().debug(
        ...
        arena.getId(), maxdetectives, maxmurderer, playersSize, arena.getArenaOption("DETECTIVE_DIVIDER"),
    ...
    if (playersSize - (maxmurderer + maxdetectives) < 1) {
      arena.getPlugin().getDebugger()
          .debug("{0} Murderers and detectives amount was reduced because there are not enough players", arena.getId());
